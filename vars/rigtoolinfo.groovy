...
def call(rig,tool)
{
 ...
 def response = sh(script:"""curl -X POST  -L -w '%{http_code}\n' -H  Accept:application/json -H  Content-Type:application/json  -d '{ \"rigletName\":\"${rig}\", \"toolName\":\"${tool}\"}' http://3.134.156.211:3013/aapi/riglets/connectorServerDetails -o rigoutput.json""", returnStdout: true)
 println(response)
